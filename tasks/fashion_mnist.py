"""Fashion-MNIST data loading, training loop, and evaluation."""
...
def make_dataloaders(batch_size: int = 128) -> tuple[DataLoader, DataLoader]:
    """Create Fashion-MNIST train/val dataloaders."""
    # Standard Fashion-MNIST normalization constants
    mean = (0.2860,)
    std = (0.2267,)
    ...
    train = datasets.FashionMNIST("./data", train=True, download=True, transform=train_transform)
    test = datasets.FashionMNIST("./data", train=False, download=True, transform=test_transform)
...
def train_one_epoch(model: torch.nn.Module, loader: DataLoader, optimizer: torch.optim.Optimizer, device: torch.device) -> float:
    # This is identical to MNIST version; keeping for standalone usability or moving to utils later
    model.train()
    total_loss = 0.0
    correct = 0
    total = 0
...
def evaluate(model: torch.nn.Module, loader: DataLoader, device: torch.device) -> tuple[float, float]:
    # This is identical to MNIST version
    model.eval()
    total_loss = 0.0
    correct = 0
    total = 0
...
def run_experiment(
    ...
    """Run a full Fashion-MNIST training experiment."""
