...
RESULTS_FILE = "./plots/fig2_r2_results.json"
...
class NoisyLocalCellRegressionDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        # Clean lognormal signal: log(x_i) ~ N(0, sigma^2), so x_i > 0
        log_feat = torch.randn(self.seq_len, generator=self.rng) * self.sigma
        x_clean = torch.exp(log_feat)

        # Target from clean x: mean over cell geometric means
        ...
        # Additive Gaussian noise only to X
...
def make_noisy_local_reg_dataloaders(seq_len: int, seed: int):
...
if __name__ == "__main__":
    main()
    # nohup python collect_fig2_r2_data.py > plots/fig2_r2.log 2>&1 &
