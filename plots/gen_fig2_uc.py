...
B3_RESULTS_FILE = "./plots/fig2_b3_uc_results.json"
R2_RESULTS_FILE = "./plots/fig2_r2_uc_results.json"
...
def load_b3_data() -> np.ndarray:
    ...
    data = np.zeros((3, len(UNIT_CELLS), len(SEEDS)))
    for pi, pooling in enumerate(POOLINGS):
        for li, unit_cell in enumerate(UNIT_CELLS):
...
def plot_combined(b3_data: np.ndarray, r2_data: np.ndarray):
    ...
    ax_bottom.set_xlabel(r"Unit cell $M$", fontsize=12)
    ...
    ax_bottom.set_xlim(1, 11)
    ax_bottom.set_xticks([2, 4, 6, 8, 10])
    ax_bottom.set_xticklabels([2, 4, 6, 8, 10], fontsize=12)
    ...
    out_path = os.path.join(fig_dir, "..", "plots", "fig2_uc.pdf")
