...
from models.cnn import CIFARCNNAll, CIFARCNNLocal, CIFARCNNGlobal
from tasks.cifar10 import make_dataloaders, train_one_epoch, evaluate
...
def run_experiment(model_class, pooling: str, epochs=100, lr=0.001, batch_size=128, seed=42):
    ...
    print(f"\n--- Running CIFAR-10 | Model: {model_name} | Pooling: {pooling} ---")
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 3:
        print("Usage: python run_cifar10.py <model> <pooling>")
    ...
    acc = run_experiment(model_cls, p_arg, epochs=200, lr=0.001)
    print("\n" + "="*30)
    print(f"CIFAR-10 | {m_arg} | {p_arg}: {acc:.4f}")
    print("="*30)
